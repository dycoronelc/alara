...
export class PdfService {
  ...
  /** Misma altura que `drawReportSectionBar` sin pintar (para reservar espacio). */
  private measureReportSectionBarHeight(doc: PDFKit.PDFDocument, barWidth: number, title: string): number {
    const padV = 7;
    const text = title.toUpperCase();
    doc.font('ReportTitle').fontSize(12);
  ...
  /** Altura de la primera fila de campos (1 celda ancha o 2 columnas). */
  ...
  private renderTemplateSection(
    ...
    const fields = section.fields.filter((f) => {
      ...
      if (f.key === 'socios_participacion') {
        const role = (
          valuesByKey.get('employee_or_partner') ??
      ...
      if (f.key === 'negative_history_detail') {
        const v = (
          valuesByKey.get('negative_history') ??
